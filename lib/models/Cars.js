const pool = require('../utils/pool');
const Driver = require('./Drivers');

module.exports = class Car {
    id;
    make;
    model;
    color;

    constructor(row) {
      this.id = row.id;
      this.make = row.make;
      this.model = row.model;
      this.color = row.color;
    }

    static async insert({ make, model, color }) {
      const { rows } = await pool.query(
        'INSERT INTO cars (make, model, color) VALUES ($1, $2, $3) RETURNING *',
        [make, model, color]
      );

      return new Car(rows[0]);
    }

    static async findById(id) {
      const { rows } = await pool.query(
        `SELECT 
          cars.*,
          array_to_json(array_agg(drivers.*)) AS drivers 
        FROM 
          cars
        JOIN drivers
        ON cars.id = drivers.car_id
        WHERE cars.id=$1
        GROUP BY cars.id`,
        [id]
      );

      if(!rows[0]) throw new Error(`No car found with the id of ${id}`);

      return { 
        ...new Car(rows[0]),
        drivers: rows[0].drivers.map(drivers => new Driver(drivers))
      };
    }


    static async find() {
      const { rows } = await pool.query('SELECT * FROM cars');

      return rows.map(row => new Car(row));
    }
    
    static async update(id, { make, model, color }) {
      const { rows } = await pool.query(
        `UPDATE cars
          SET make=$1,
            model=$2,
            color=$3
            WHERE id=$4
            RETURNING *`,
        [make, model, color, id]
      );
  
      if(!rows[0]) throw new Error(`No car found with the id of ${id}`);
        
      return new Car(rows[0]);
    }


    static async delete(id) {
      const { rows } = await pool.query(
        'DELETE FROM cars WHERE id=$1 RETURNING *',
        [id]
      );
  
      if(!rows[0]) throw new Error(`No car found with the id of ${id}`);
        
      return new Car(rows[0]);
    }
};
